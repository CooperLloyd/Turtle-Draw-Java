...
public class Turtle {
	...
	public Turtle() {
		this.pen = new pen();
		this.position= new Point2(0.0, 0.0);
		this.heading= (0.0);
	}
	 ...
	 */
	public Turtle(Turtle other) { // creates a clone of a turtle object
		this.pen = other.pen;
	...
	public Turtle(Point2 position, double heading, Color c) {
		this.pen = new pen();
		if(c == null){
			throw new NullPointerException("pen color is null");
		}
		else{
			this.setPenColor(c);
		}
	...
	private void walk(double distance) {
		...
		// Theta value to be used in cos and sin functions
		double theta = Math.toRadians(this.heading);

		// Calculate new x and y positions
		double vec_x = distance * Math.cos(theta);
		double vec_y = distance * Math.sin(theta);

		// New vector representation of x and y pos
		Vector2 v = new Vector2(vec_x, vec_y);

		// Add v to the class attribute position of type Point2
	...
	private static double wrapAngle(double degrees) {
		...
		wrapped = Math.floorMod((int) degrees, 360);
		if(wrapped<0){
			wrapped+=360;
		}
		return wrapped;
	...
	public void turnLeft(double delta) {
		if(delta<0){
			throw new IllegalArgumentException("delta cant be less than 0");
		}
		else{
